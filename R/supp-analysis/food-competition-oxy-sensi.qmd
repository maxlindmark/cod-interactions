...
for(i in unique(d$group)) {
  ...
  # Saduria model
    ...
    m_sad <- sdmTMB(saduria_rel_weight ~ 0 + fyear + fquarter + depth_sc + oxygen_sc + 
                      ...
                      flounder_density_sc*density_saduria_sc + pred_length_cm_sc,
                    data = dd,
                    mesh = mesh,
                    family = tweedie(),
                    weights = weigths, 
    ...
    if(unique(dd$group) %in% c("large cod", "small cod")) {
      ...
      m_ben_ctd <- sdmTMB(benthic_rel_weight ~ 0 + fyear + fquarter + depth_sc + 
                            small_cod_density_sc*oxygen_ctd_sc + flounder_density_sc*oxygen_ctd_sc + pred_length_cm_sc,
                          data = dd,
                          mesh = mesh,
                          family = tweedie(),
                          weights = weigths, 
  ...
  # Coefficients
  coefs_sad <- tidy(m_sad, effects = "fixed", conf.int = TRUE) |> 
    mutate(species = "Cod (m)",
           response = "Saduria",
           sig = ifelse(estimate > 0 & conf.low > 0, "Y", "N"),
           sig = ifelse(estimate < 0 & conf.high < 0, "Y", sig))
  
  coefs_sad_ctd <- tidy(m_sad_ctd, effects = "fixed", conf.int = TRUE) |> 
    mutate(species = "Cod (m)",
           response = "Saduria",
           sig = ifelse(estimate > 0 & conf.low > 0, "Y", "N"),
           sig = ifelse(estimate < 0 & conf.high < 0, "Y", sig))
  
  coefs_ben <- tidy(m_ben, effects = "fixed", conf.int = TRUE) |> 
    mutate(species = "Cod (m)",
           response = "Saduria",
           sig = ifelse(estimate > 0 & conf.low > 0, "Y", "N"),
           sig = ifelse(estimate < 0 & conf.high < 0, "Y", sig))
  
  coefs_ben_ctd <- tidy(m_ben_ctd, effects = "fixed", conf.int = TRUE) |> 
    mutate(species = "Cod (m)",
           response = "Saduria",
           sig = ifelse(estimate > 0 & conf.low > 0, "Y", "N"),
           sig = ifelse(estimate < 0 & conf.high < 0, "Y", sig))
...
```{r}
  ...
  m_tot <- sdmTMB(tot_rel_weight ~ 0 + fyear + fquarter + depth_sc + pred_length_cm_sc +
                  ...
                  mesh = mesh,
                  family = tweedie(),
                  weights = dd$weights_lc,
                  spatiotemporal = "iid", 
                  spatial = "off",
                  time = "year")
  
  m_tot_ctd <- sdmTMB(tot_rel_weight ~ 0 + fyear + fquarter + depth_sc + pred_length_cm_sc +
                    large_cod_density_sc*oxygen_ctd_sc + flounder_density_sc*oxygen_ctd_sc +
                        pred_length_cm_sc,
                  data = dd,
                  mesh = mesh,
                  family = tweedie(),
  ...
  coefs_tot_ctd <- tidy(m_tot_ctd, effects = "fixed", conf.int = TRUE) |> 
    mutate(response = "Total",
           sig = ifelse(estimate > 0 & conf.low > 0, "Y", "N"),
           sig = ifelse(estimate < 0 & conf.high < 0, "Y", sig)) |> 
    mutate(group = "large cod")
...
coef_df <- bind_rows(bind_rows(coef_sad) |> mutate(model = "Saduria", oxy_source = "model"),
                     ...
                     bind_rows(coefs_tot) |> mutate(model = "Total prey", oxy_source = "model"),
                     bind_rows(coefs_tot_ctd) |> mutate(model = "Total prey", oxy_source = "ctd"))
```
...
coef_df2 <- coef_df |>
  ...
  mutate(term = str_remove_all(term, "_sc"),
         ...
         term = ifelse(term == "Oxygen ctd", "Oxygen", term),
         term = ifelse(term == "Small cod ×oxygen ctd", "Small cod × oxygen", term),
         term = ifelse(term == "Large cod ×oxygen ctd", "Large cod × oxygen", term),
         ...
         oxy_source = ifelse(oxy_source == "model", "Model", "CTD"),
...
p1 <- 
  ...
  ggplot(aes(estimate, term, color = oxy_source, alpha = sig2)) +
  geom_stripped_rows(aes(y = term), inherit.aes = FALSE) +
  facet_grid(factor(model2, levels = c("Prey=Saduria", "Prey=Benthic prey")) ~ factor(group, levels = c("Flounder", "Small cod", "Large cod"))) +
  ...
  labs(x = "Estimate", y = "", alpha = "95% CI crossing 0", color = "Oxygen source") +
...
p3 <- 
  ...
  ggplot(aes(estimate, term, color = oxy_source, alpha = sig2)) +
  geom_stripped_rows(aes(y = term), inherit.aes = FALSE) +
  facet_grid(model2 ~ factor(group2, levels = c("Predator=Large cod"))) +
  geom_vline(xintercept = 0, linetype = 2, alpha = 0.5, color = "gray10", linewidth = 0.2) +
  geom_point(position = position_dodge(width = 0.5), size = 2) +
  geom_errorbar(aes(xmin = conf.low, xmax = conf.high), width = 0,
                position = position_dodge(width = 0.5)) +
...
ggsave(paste0(home, "/figures/supp/coefs_oxy_sens.pdf"), width = 18, height = 22, units = "cm")
```
