...
mcod1 <- sdmTMB(density_cod ~ 1 + density_fle_sc*depth_sc + depth_sc_sq,
...
# Save model
saveRDS(mcod1, "output/mcod1.rds")
# mcod1 <- readRDS("output/mcod1.rds")
```
...
mfle1 <- sdmTMB(density_fle ~ 1 + density_cod_sc*depth_sc + depth_sc_sq,
                data = d, spde = spde, family = tweedie(link = "log"),
                fields = "AR1", include_spatial = TRUE, time = "year",
                spatial_only = FALSE, spatial_trend = TRUE, reml = FALSE,
                control = sdmTMBcontrol(newton_steps = 1))
...
saveRDS(mfle1, "output/mfle1.rds")
# mfle1 <- readRDS("output/mfle1.rds")
...
```{r fit fle depth covariate, cache=TRUE}
