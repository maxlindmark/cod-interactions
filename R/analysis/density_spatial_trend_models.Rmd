...
# To load entire cache in interactive r session, do: qwraps2::lazyload_cache_dir(path = "R/analysis/density_spatial_trend_models_cache/html")
```
...
d1 <- d %>% filter(quarter == 1)
d4 <- d %>% filter(quarter == 4)
```
...
spde_q1 <- make_mesh(d1, xy_cols = c("X", "Y"),
                     n_knots = 100, type = "kmeans", seed = 42)

spde_q4 <- make_mesh(d4, xy_cols = c("X", "Y"),
                     n_knots = 100, type = "kmeans", seed = 42)

# Plot and save spde
png(file = "figures/supp/density_spde_q1.png", units = "in", width = 6.5, height = 6.5, res = 300)
plot(spde_q1)
dev.off()

png(file = "figures/supp/density_spde_q4.png", units = "in", width = 6.5, height = 6.5, res = 300)
plot(spde_q4)
dev.off()
```
...
```{r fit cod covariates and field q1, message=FALSE, cache=TRUE}
mcod1_q1 <- sdmTMB(density_cod ~ 1 + density_fle_sc + s(depth_sc),
                   data = d1, spde = spde_q1,
...
tidy(mcod1_q1, conf.int = TRUE)

d1$residualsmcod1_q1 <- residuals(mcod1_q1)
qqnorm(d1$residualsmcod1_q1); abline(a = 0, b = 1)

# Save model
saveRDS(mcod1_q1, "output/mcod1_q1.rds")
# mcod1_q1 <- readRDS("output/mcod1_q1.rds")

# Check AR1 param
mcodsd1_q1 <- as.data.frame(summary(TMB::sdreport(mcod1_q1$tmb_obj)))
mcodsd1_q1$Estimate[row.names(mcodsd1_q1) == "ar1_phi"]
mcodsd1_q1$Estimate[row.names(mcodsd1_q1) == "ar1_phi"] +
  c(-2, 2) * mcodsd1_q1$`Std. Error`[row.names(mcodsd1_q1) == "ar1_phi"]
```

```{r fit cod covariates and field q4, results='hide', cache=TRUE, message=FALSE}
mcod1_q4 <- sdmTMB(density_cod ~ 1 + density_fle_sc + s(depth_sc),
                   data = d4, spde = spde_q4,
                   family = tweedie(link = "log"),
                   fields = "AR1", include_spatial = TRUE, time = "year",
                   spatial_only = FALSE, spatial_trend = TRUE, reml = FALSE,
                   control = sdmTMBcontrol(newton_steps = 1))

tidy(mcod1_q4, conf.int = TRUE)

d4$residualsmcod1_q4 <- residuals(mcod1_q4)
qqnorm(d4$residualsmcod1_q4); abline(a = 0, b = 1)

# Save model
saveRDS(mcod1_q4, "output/mcod1_q4.rds")
# mcod1_q4 <- readRDS("output/mcod1_q4.rds")
```

#### Only depth covariate

```{r fit cod depth covariate q1, results='hide', cache=TRUE, message=FALSE}
mcod2_q1 <- sdmTMB(density_cod ~ 1 + s(depth_sc),
                   data = d1, spde = spde_q1,
                   family = tweedie(link = "log"),
                   fields = "AR1", include_spatial = TRUE, time = "year",
                   spatial_only = FALSE, spatial_trend = TRUE, reml = FALSE,
...
tidy(mcod2_q1, conf.int = TRUE)

d1$residualsmcod2_q1 <- residuals(mcod2_q1)
qqnorm(d1$residualsmcod2_q1); abline(a = 0, b = 1)

# Save model
saveRDS(mcod2_q1, "output/mcod2_q1.rds")
# mcod2_q1 <- readRDS("output/mcod2_q1.rds")
```

```{r fit cod depth covariate q4, results='hide', cache=TRUE, message=FALSE}
mcod2_q4 <- sdmTMB(density_cod ~ 1 + s(depth_sc),
                   data = d4, spde = spde_q4,
                   family = tweedie(link = "log"),
                   fields = "AR1", include_spatial = TRUE, time = "year",
                   spatial_only = FALSE, spatial_trend = TRUE, reml = FALSE,
...
tidy(mcod2_q4, conf.int = TRUE)

d4$residualsmcod2_q4 <- residuals(mcod2_q4)
qqnorm(d4$residualsmcod2_q4); abline(a = 0, b = 1)

# Save model
saveRDS(mcod2_q4, "output/mcod2_q4.rds")
...
```{r fit fle covariates and field q1, cache=TRUE}
mfle1_q1 <- sdmTMB(density_fle ~ 1 + density_cod_sc + s(depth_sc),
                   data = d1, spde = spde_q1,
                   family = tweedie(link = "log"),
                   fields = "AR1", include_spatial = TRUE, time = "year",
                   spatial_only = FALSE, spatial_trend = TRUE, reml = FALSE,
...
tidy(mfle1_q1, conf.int = TRUE)

d1$residualsmfle1_q1 <- residuals(mfle1_q1)
qqnorm(d1$residualsmfle1_q1); abline(a = 0, b = 1)

saveRDS(mfle1_q1, "output/mfle1_q1.rds")
# mfle1_q1 <- readRDS("output/mfle1_q1.rds")

# Check AR1 param
mflesd1_q1 <- as.data.frame(summary(TMB::sdreport(mfle1_q1$tmb_obj)))
mflesd1_q1$Estimate[row.names(mflesd1_q1) == "ar1_phi"]
mflesd1_q1$Estimate[row.names(mflesd1_q1) == "ar1_phi"] +
  c(-2, 2) * mflesd1_q1$`Std. Error`[row.names(mflesd1_q1) == "ar1_phi"]
```

```{r fit fle covariates and field q4, cache=TRUE}
mfle1_q4 <- sdmTMB(density_fle ~ 1 + density_cod_sc + s(depth_sc),
                   data = d4, spde = spde_q4,
                   family = tweedie(link = "log"),
                   fields = "AR1", include_spatial = TRUE, time = "year",
                   spatial_only = FALSE, spatial_trend = TRUE, reml = FALSE,
...
tidy(mfle1_q4, conf.int = TRUE)

d4$residualsmfle1_q4 <- residuals(mfle1_q4)
qqnorm(d4$residualsmfle1_q4); abline(a = 0, b = 1)

saveRDS(mfle1_q4, "output/mfle1_q4.rds")
# mfle1_q4 <- readRDS("output/mfle1_q4.rds")
```

#### Only depth covariate

```{r fit fle depth covariate q1, cache=TRUE}
mfle2_q1 <- sdmTMB(density_fle ~ 1 + s(depth_sc),
                data = d1, spde = spde_q1,
                family = tweedie(link = "log"),
                fields = "AR1", include_spatial = TRUE, time = "year",
                spatial_only = FALSE, spatial_trend = TRUE, reml = FALSE,
...
tidy(mfle2_q1, conf.int = TRUE)

d1$residualsmfle2_q1 <- residuals(mfle2_q1)
qqnorm(d1$residualsmfle2_q1); abline(a = 0, b = 1)

saveRDS(mfle2_q1, "output/mfle2_q1.rds")
# mfle2_q1 <- readRDS("output/mfle2_q1.rds")
```

```{r fit fle depth covariate q4, cache=TRUE}
mfle2_q4 <- sdmTMB(density_fle ~ 1 + s(depth_sc),
                data = d4, spde = spde_q4,
                family = tweedie(link = "log"),
                fields = "AR1", include_spatial = TRUE, time = "year",
                spatial_only = FALSE, spatial_trend = TRUE, reml = FALSE,
...
tidy(mfle2_q4, conf.int = TRUE)

d4$residualsmfle2_q4 <- residuals(mfle2_q4)
qqnorm(d4$residualsmfle2_q4); abline(a = 0, b = 1)

saveRDS(mfle2_q4, "output/mfle2_q4.rds")
...
```{r effect sizes}
mfle1_q1_ef <- tidy(mfle1_q1, effects = "fixed", conf.int = TRUE) %>% 
  mutate(Quarter = 1, Species = "Flounder")

mfle1_q4_ef <- tidy(mfle1_q4, effects = "fixed", conf.int = TRUE) %>% 
  mutate(Quarter = 4, Species = "Flounder")

mcod1_q1_ef <- tidy(mcod1_q1, effects = "fixed", conf.int = TRUE) %>% 
  mutate(Quarter = 1, Species = "Cod")

mcod1_q4_ef <- tidy(mcod1_q4, effects = "fixed", conf.int = TRUE) %>% 
  mutate(Quarter = 4, Species = "Cod")

plot_df <- bind_rows(mfle1_q1_ef, mfle1_q4_ef, mcod1_q1_ef, mcod1_q4_ef)

# Plot effects
ggplot(filter(plot_df, !term == "(Intercept)"),
       aes(term, estimate, color = factor(Species))) +
  ...
  labs(x = "Density", y = "Standardized coefficient", color = "Quarter") +
...
ggsave("figures/spatial_trends.png", width = 6.5, height = 6.5, dpi = 600)
```
...
## Do we even have negative covariance?
...
ggsave("figures/supp/proportion_all_years.png", width = 6.5, height = 6.5, dpi = 600)
...
ggsave("figures/proportion_fle_map.png", width = 6.5, height = 6.5, dpi = 600)
