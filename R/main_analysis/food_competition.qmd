...
ben_omega <- plot_map_fc +
  geom_point(data = random_df %>% filter(model == "Benthos"), aes(X*1000, Y*1000, color = omega_s), size = 0.9) +
  scale_color_gradient2() +
  facet_wrap(~ group) +
  labs(color = "Spatial\nrandom effect") +
  theme(axis.text.x = element_text(angle = 90),
...
ggsave(paste0(home, "/figures/supp/omega.pdf"), width = 17, height = 14, units = "cm")
...
ggsave(paste0(home, "/figures/conditional_saduria_flounder.pdf"), width = 9, height = 17, units = "cm")
