...
```{r read data}
cpue_full <- readr::read_csv("data/clean/full_cpue_length_q1_q4.csv") %>%
  janitor::clean_names() %>% 
  rename(X = x,
         Y = y) 
...
plot_map_fc + 
  ...
  scale_color_gradient2(name = "Spatial random effect")

ggsave("figures/residuals_in_space.pdf", width = 20, height = 20, units = "cm")
```
...
ggsave("figures/residu_corr.pdf", width = 20, height = 20, unit = "cm")
```
