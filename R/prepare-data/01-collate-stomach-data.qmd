...
trawl_data_cod <- trawl_data |> 
  ...
  ungroup() |> 
  filter(size_group %in% c("small", "medium")) |> # could also drop NA here..
...
na_test <- trawl_data_wide |> filter(is.na(fle_kg_km2)) |> dplyr::select(haul_id) |> head(10)
...
d <- d |>
  ...
  tidylog::filter(prey_nr > 0) # FIXME
...
d2 |>
  filter(is.na(prey_weight_g)) |> 
  ggplot(aes(factor(n_regurgitated))) + 
  geom_histogram(stat = "count") +
  theme(axis.text.x = element_text(angle = 90))
...
# These are not 
d2 |> filter(prey_weight_g == 0) |> distinct(n_regurgitated)
