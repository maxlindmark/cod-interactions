...
p1 <- ggplot(ovr2, aes(mean_fle, `Flounder\nSmall cod`)) + 
  ...
  stat_cor(method = "pearson", label.x = 5700, label.y = 0.97, size = 3) +
  labs(y = "Flounder:Small cod diet overlap")

p2 <- ggplot(ovr2, aes(mean_fle, `Flounder\nLarge cod`)) + 
  geom_point() + 
  coord_cartesian(ylim = c(0, 1)) +
  stat_cor(method = "pearson", label.x = 5000, label.y = 0.97, size = 3) +
...
ggsave(paste0(home, "/figures/schoener_pearson.pdf"), width = 17, height = 8, units = "cm")
```
