...
d |>
  summarise(length(unique(pred_id)),
            .by = c("species", "quarter"))
...
dw <- d |> 
  ...
  dplyr::select(f_weight, sc_weight, lc_weight, pred_id, group) |> 
  mutate(weight = f_weight, 
         weight = ifelse(group == "Large cod", lc_weight, weight),
         weight = ifelse(group == "Small cod", sc_weight, weight)) |> 
  dplyr::select(-f_weight, -sc_weight, -lc_weight, -group)
...
d3 <- d2 |>
  ...
  dplyr::select(-scrap) |>
  summarise(tot_prey_weight = weighted.mean(value, weight), #mean(value),
...
d3 <- d2 |> 
  ...
  summarise(tot_prey_weight = weighted.mean(value, weight), .by = c(species, pred_length_cm, name)) |>
  mutate(name = str_replace(name, "_", " "),
         name = str_replace(name, "_", " "),
         name = str_remove(name, " tot"),
         name = str_to_title(name))
