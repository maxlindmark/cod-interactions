...
for(i in unique(d$group)) {
  ...
  nd_cod <- data.frame(expand_grid(
    ...
    mutate(year = 2020,
           ...
           oxygen_sc = 0,
           depth_sc = 0,
           flounder_density_sc = 0) # TODO: why do I need this when I specify re_form_iid!?
...
ggplot(coef_df3, aes(term, estimate, color = factor(group, levels = c("Flounder", "Small cod", "Large cod")), 
...
# summary(pred_df2)
# unique(pred_df2$density_saduria_sc)
# 
# pred_df2 %>%
#   filter(xvar == "flounder") %>% 
#     filter(group == "Large cod") %>%
#     filter(!model == "Saduria") %>% 
#   filter(!density_saduria_sc %in% c(min(density_saduria_sc), max(density_saduria_sc))) %>% 
#   ggplot(aes(flounder_density_sc, exp(est), color = model, fill = model)) +
#   geom_ribbon(aes(ymin = exp(est - 1.150*est_se), ymax = exp(est + 1.150*est_se)),
#               alpha = 0.3, color = NA) +
#   geom_line() +
...
#   coord_cartesian(#xlim = c(-0.5, 0.5), 
#                   expand = 0) +
#   labs(x = "Flounder density", y = "Relative prey weight",
...
#            depth_sc = 0,
#            small_cod_density_sc = 0) # TODO: why do I need this when I specify re_form_iid!?
# 
