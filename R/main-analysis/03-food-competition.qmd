...
range_df %>% 
  ...
  ggplot(aes(model, estimate, color = factor(group, levels = c("Flounder", "Small cod", "Large cod")))) + 
  geom_point(size = 2) +
...
ggplot(coef_df2, aes(estimate, term, alpha = sig,
...
ggplot(coef_df5, aes(estimate, model, color = factor(group, levels = c("Flounder", "Small cod", "Large cod")),
                     alpha = sig)) +
  geom_vline(xintercept = 0, linetype = 2, alpha = 0.5, color = "gray10", linewidth = 0.2) +
  geom_point(position = position_dodge(width = 0.5), size = 1.5) +
  geom_errorbar(aes(xmin = conf.low, xmax = conf.high), width = 0,
